"""BioInstruct dataset -- prepare and load in one module.

As a library (the registry calls this): ``load(cfg, tokenizer)`` builds
``{data_dir}/bioinstruct.jsonl`` on first use -- the BioInstruct 25k
biomedical instruction set (Tran et al., 2024) reformatted to
``{"instruction", "input", "output"}`` -- then tokenizes it. The source already
ships ``instruction`` / ``input`` / ``output`` columns, so mapping is a passthrough.
Select with ``cfg.dataset.name = bioinstruct``.

As a script: ``python -m dataset.load_bioinstruct [--dataset_name ...]
[--data_files ...] [--sample_percentage ...]`` (re)builds just the JSONL.

The source defaults to the Hub (``bio-nlp-umass/bioinstruct``). Override via
...
DEFAULT_SOURCE = "bio-nlp-umass/bioinstruct"
FILENAME = "bioinstruct.jsonl"


def to_record(example):
    """Map one BioInstruct row to ``{instruction, input, output}`` or ``None``."""
...
def prepare(cfg):
    """Build ``{data_dir}/bioinstruct.jsonl`` if missing; return its path."""
    ...
    if not os.path.exists(path):
        ...
        print(f"[bioinstruct] wrote {len(records)} examples to {path}")
    return path
...
def _cli():
    p = argparse.ArgumentParser(description="Build the BioInstruct JSONL.")
